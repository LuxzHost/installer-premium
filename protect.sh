...
# ==================================================
# PROTECT BY LUXZ - PANEL PROTECTION SCRIPT
# Created by: LuxzOfficial
# Version: 1.0
# ==================================================
...
echo -e "${BLUE}"
echo "=================================================="
echo "           PROTECT BY LUXZ INSTALLER"
echo "           Created by Luxz Official"
echo "=================================================="
echo -e "${NC}"
...
# Backup directory
BACKUP_DIR="/var/www/pterodactyl/backups/protect_luxz_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$BACKUP_DIR"
    ...
    public function handle(Server $server): void
    {
        ...
        if ($user) {
            if ($user->id !== 1) {
                ...
                if ($ownerId !== $user->id) {
                    throw new DisplayException('Akses ditolak: Anda hanya dapat menghapus server milik Anda sendiri @ PROTECT BY Luxz - Luxz Official');
                }
            }
...
class UserController extends Controller
{
    ...
    public function delete(Request $request, User $user): RedirectResponse
    {
        ...
        if ($request->user()->id !== 1) {
            throw new DisplayException("❌ Hanya admin ID 1 yang dapat menghapus user lain! © Protect by Luxz - Luxz Official V1.0");
        }
        // ============================================
    ...
    public function update(UserFormRequest $request, User $user): RedirectResponse
    {
        ...
        foreach ($restrictedFields as $field) {
            if ($request->filled($field) && $request->user()->id !== 1) {
                throw new DisplayException("⚠️ Data hanya bisa diubah oleh admin ID 1. © Protect by Luxz - Luxz Official V1.0");
            }
        }

        // Cegah turunkan level admin ke user biasa
        if ($user->root_admin && $request->user()->id !== 1) {
            throw new DisplayException("🚫 Tidak dapat menurunkan hak admin pengguna ini. Hanya ID 1 yang memiliki izin. © PROTECT BY Luxz - Luxz Official V1.0");
        }
        // ====================================================
...
class LocationController extends Controller
{
    ...
    public function index(): View
    {
        ...
        $user = Auth::user();
        if (!$user || $user->id !== 1) {
            abort(403, 'Protect by Luxz - Akses ditolak');
        }
...
class NodeController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        $user = Auth::user();
        if (!$user || $user->id !== 1) {
            abort(403, '🚫 Akses ditolak! Hanya admin ID 1 yang dapat membuka menu Nodes. © Protect by Luxz - Luxz Official V1.0');
        }
        // ======================================================
...
class NestController extends Controller
{
    ...
    public function index(): View
    {
        ...
        $user = Auth::user();
        if (!$user || $user->id !== 1) {
            abort(403, '🚫 Akses ditolak! Hanya admin utama (ID 1) yang bisa membuka menu Nests. © Protect by Luxz - Luxz Official V1.0');
        }
...
echo -e "${BLUE}Starting Protect by Luxz installation...${NC}"
echo ""
...
echo ""
echo -e "${GREEN}==================================================${NC}"
echo -e "${GREEN}    PROTECT BY LUXZ INSTALLATION COMPLETE!${NC}"
echo -e "${GREEN}            Created by Luxz Official${NC}"
echo -e "${GREEN}==================================================${NC}"
echo ""
...
echo ""
echo -e "${BLUE}Your Pterodactyl panel is now protected by Luxz!${NC}"
echo ""
